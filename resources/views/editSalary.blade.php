...
<ol class="breadcrumb breadcrumb-col-blue pull-right">
        ...
        <li class="active"><i class="material-icons">edit</i> Edit Salary</li>
</ol>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card" style="width: 580px; margin-left: 300px">
                        <div class="header bg-blue">
                            <h2>
                              Edit Employee's Salary: &nbsp;<small>(Get employee's salary details here)</small>
                            </h2>
                        ...
                        <div class="body">
                            <form action="{{ url('/edit_salary') }}" method="post">
							@csrf
...
-->
									...
									<div class="form-group">
										<div class="form-line">
										   <select class="form-control {{ $errors->has('month') ? ' is-invalid' : '' }}" name="month" id="month">
												<option value="" id="default">Select Employee Name first to select month</option>
											</select>
										</div>
										 @if ($errors->has('month'))
										<span class="invalid-feedback" role="alert" style="color: red;">
							...
							<script>
								...
								$('#emp_name').on('change', function(event) {
									var x = $(this).val();
									//console.log(x);
									$.get("{{ url('api/getEmpMonthDetails')}}", 
										  { id: x }, 
										  function(data) {
										//console.log(data);
										$.each(data, function(index, element) {
											var option = $('<option value="'+ element.cid +'">'+ element.month +'</option>');
											$('#month').append(option);
//											 $('#month')
//												 .append($("<option></option>")
//														 .attr("value",element.cid)
//														 .text(element.month)); 
											$('#default').text('Select Month here');
											$('#month').selectpicker('refresh');
											//console.log($('#month').html());
											$('#basic_sal').val(element.salary);
								 		});		
...
//											$('#pay_sal').val($('#net_sal').val());
								...
								$('#month').on('change', function(event) {
									...
									$.get("{{ url('api/getMonthDetails')}}", 
										...
										$.each(data, function(index, element) {
											...
											$('#pay_sal').val(element.pay_sal);
											$('#adv_paid').val(element.adv_paid);
											$('#sal_encash').val(element.sal_encash);
											$('#tds_deduct').val(element.tds_deduct);
											$('#sal_id').val(element.sal_id);
											console.log($('#sal_id').val());
											var d= element.leave_DHM.split('d').shift();
											$('#ldays').val(d);
											var h= element.leave_DHM.substr(0, element.leave_DHM.indexOf('h')); 
											var hl= h.substr(7);  
											$('#lhours').val(hl);
											var m= element.leave_DHM.substr(0, element.leave_DHM.indexOf('m')); 
											var ml= m.substr(16); 
											$('#lmins').val(ml);
								 		});
								});
								...
								$('#ldays').on('keyup', function(event) {
									leave_days();
								})
								
								$('#lhours').on('keyup', function(event) {
									leave_hours();
								})
								
								$('#lmins').on('keyup', function(event) {
									leave_mins();
								})
